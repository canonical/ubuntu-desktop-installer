...
class GeoService {
  ...
  Future<Iterable<GeoLocation>> searchCoordinates(LatLng coordinates) async {
    ...
    log.debug('Nearest location to $coordinates: ${locations.firstOrNull}');
    return locations;
  }
...
abstract class GeoSource {
  ...
  /// Looks up the sources and returns locations near the [coordinates], if any.
  Future<Iterable<GeoLocation>> searchCoordinates(LatLng coordinates);
...
class Geodata implements GeoSource {
  ...
  Future<Iterable<GeoLocation>> searchCoordinates(LatLng coordinates) async {
    await _ensureInitialized();
    return _coordinates.knn(coordinates.longitude, coordinates.latitude, 1);
  }
...
class GeoBush extends RBushBase<GeoLocation> {
  GeoBush()
      : super(
