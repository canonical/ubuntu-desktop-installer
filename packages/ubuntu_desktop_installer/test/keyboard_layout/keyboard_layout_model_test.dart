...
void main() {
  ...
  group('layout and variant', () {
    ...
    test('selection applies keyboard settings', () async {
      await model.selectLayout(0);
      verify(client.setKeyboard(const KeyboardSetting(layout: 'foo')))
          .called(1);

      await model.selectLayout(1);
      verify(client.setKeyboard(
              const KeyboardSetting(layout: 'bar', variant: 'baz')))
          .called(1);

      await model.selectVariant(1);
      verify(client.setKeyboard(
              const KeyboardSetting(layout: 'bar', variant: 'qux')))
          .called(1);
    });
  ...
  test('apply the system settings', () async {
    ...
    verify(client
            .setKeyboard(const KeyboardSetting(layout: 'bar', variant: 'qux')))
        .called(1);
  });
}
