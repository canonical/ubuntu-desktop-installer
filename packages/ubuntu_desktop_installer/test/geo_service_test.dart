...
void main() {
  ...
  test('cancel search', () async {
    final source1 = MockGeoSource();
    when(source1.search('foo')).thenAnswer((_) async => [gothenburg]);
    when(source1.cancel()).thenAnswer((_) async {});

    final source2 = MockGeoSource();
    when(source2.search('foo')).thenAnswer((_) async => [gothenburg]);
  ...
  test('geoname error', () async {
    ...
    expect(await geoname.search('foo'), isEmpty);
  });
  ...
  test('geodata search', () async {
    ...
    bool isStavanger(result) =>
        result.name == 'Stavanger' &&
        result.admin == null &&
        result.country == 'Norway' &&
        result.country2 == 'NO';
    ...
    await geodata.search('o').then((results) {
      expect(results, isNot(contains(predicate<GeoLocation>(isHelsinki))));
      expect(results, contains(predicate<GeoLocation>(isOslo)));
      expect(results, isNot(contains(predicate<GeoLocation>(isStadi))));
      expect(results, isNot(contains(predicate<GeoLocation>(isStavanger))));
      expect(results, isNot(contains(predicate<GeoLocation>(isStockholm))));
    });

    await geodata.search('ST').then((results) {
      expect(results, isNot(contains(predicate<GeoLocation>(isHelsinki))));
      expect(results, isNot(contains(predicate<GeoLocation>(isOslo))));
      expect(results, contains(predicate<GeoLocation>(isStadi)));
      expect(results, contains(predicate<GeoLocation>(isStavanger)));
      expect(results, contains(predicate<GeoLocation>(isStockholm)));
    });

    await geodata.search('Stock').then((results) {
      expect(results, isNot(contains(predicate<GeoLocation>(isHelsinki))));
      expect(results, isNot(contains(predicate<GeoLocation>(isOslo))));
      expect(results, isNot(contains(predicate<GeoLocation>(isStadi))));
      expect(results, isNot(contains(predicate<GeoLocation>(isStavanger))));
    ...
    await geodata.search('uusi').then((results) {
    ...
    await geodata.search('Hel uus FIN').then((results) {
      expect(results, contains(predicate<GeoLocation>(isHelsinki)));
      expect(results, isNot(contains(predicate<GeoLocation>(isOslo))));
      expect(results, isNot(contains(predicate<GeoLocation>(isStadi))));
      expect(results, isNot(contains(predicate<GeoLocation>(isStavanger))));
    ...
    await geodata.search('Hel s FIN').then((result) => expect(result, isEmpty));
  });
